...
class MergingModel(db: Database)(implicit ec: ExecutionContext) {
    ...
    def updateStudent(classId: Int, studentId: Int, hash: String): Future[Boolean] = {
        // it is possible that there exists a friendreported name with the target hash
        // we first look for it in the database
        val studentWithCorrectNameId: Future[Option[Int]] = studentModel.getByHash(hash, classId)
        studentWithCorrectNameId.flatMap(frID =>{
            if (!frID.isEmpty){
                ...
                val success: Future[Boolean] = rewireAndDelete(frID.get, studentId) // todo: this could return false and is currently not handled
            }
            changeName(studentId, hash)
        })
    }
    ...
    def rewireAndDelete(friendId: Int, selfId: Int): Future[Boolean] = {
                                                                    // old,     new
        val success: Future[Boolean] = relationModel.rewireRelations(friendId, selfId)
    ...
    def changeName(studentId: Int, newHashedName: String): Future[Boolean] = {
        val affectedRows: Future[Int] = db.run(Student.filter(_.id === studentId).map(row => (row.hashedname)).update(newHashedName))
