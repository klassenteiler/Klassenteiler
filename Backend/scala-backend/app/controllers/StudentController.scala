...
class StudentController @Inject() (
    ...
    implicit request: Request[AnyContent] =>
      ...
      val body = { _: ClassTeacherCC =>
        model
          ...
          .map(number => Ok(Json.obj("value" -> number)))
      } //return
  ...
  def getSelfReportedStudents(implicit
      id: Int,
      classSecret: String
  ): play.api.mvc.Action[play.api.mvc.AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
      val body = { _: ClassTeacherCC =>
        model
          .getAllSelfReportedStudents(id)
          .map(students => Ok(Json.toJson(students)))
      } //return

      auth.withTeacherAuthentication(body)
  }

  def getFriendReportedStudents(implicit
      id: Int,
      classSecret: String
  ): play.api.mvc.Action[play.api.mvc.AnyContent] = Action.async {
    implicit request: Request[AnyContent] =>
      val body = { _: ClassTeacherCC =>
        model
          .getAllFriendReportedStudents(id)
          .map(students => Ok(Json.toJson(students)))
      } //return
